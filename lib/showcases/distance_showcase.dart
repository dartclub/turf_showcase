...
class DistanceShowcase extends StatefulWidget {
  const DistanceShowcase({super.key});

  @override
  State<DistanceShowcase> createState() => _DistanceShowcaseState();
}

class _DistanceShowcaseState extends State<DistanceShowcase> {
  ...
  double get _distance {
    ...
    return distance(f, t, _unit).toDouble();
  }
  ...
  Widget build(BuildContext context) {
    final mid = LatLng(
      (_from.latitude + _to.latitude) / 2,
      (_from.longitude + _to.longitude) / 2,
    );
    return ShowcaseFrame(
      hint: 'Drag the points to recalculate the haversine distance',
      map: fm.FlutterMap(
            ...
            fm.Marker(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: ShowcaseColors.sun.withOpacity(0.5)),
                ),
                child: Center(
                  child: Text(
                    '${_distance.toStringAsFixed(1)} $_unitLabel',
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<turf.Unit>(
          ...
          items: const [
            DropdownMenuItem(value: turf.Unit.kilometers, child: Text('Unit.kilometers')),
            DropdownMenuItem(value: turf.Unit.miles, child: Text('Unit.miles')),
            DropdownMenuItem(value: turf.Unit.meters, child: Text('Unit.meters')),
            DropdownMenuItem(value: turf.Unit.nauticalmiles, child: Text('Unit.nauticalmiles')),
          ],
          onChanged: (u) {
            if (u == null) return;
            setState(() => _unit = u);
